...
BOOL CD3DVidRender::InitD3D_YUV(HWND hwnd, int img_width, int img_height)
{
    ...
    lRet = m_pDirect3DDevice->CreateOffscreenPlainSurface(img_width, img_height, (D3DFORMAT)MAKEFOURCC('Y', 'V', '1', '2'),
...
BOOL CD3DVidRender::Render_YUV(unsigned char* pdata, int img_width, int img_height)
{
    ...
    {
        memmove(pDest + stride * img_height + i * stride / 2,
                pSrc + img_width * img_height + img_width * img_height / 4 + i * img_width / 2, img_width / 2);
    }

    for(i = 0; i < img_height / 2; i ++)
    {
        memmove(pDest + stride * img_height + stride * img_height / 4 + i * stride / 2,
                pSrc + img_width * img_height + i * img_width / 2, img_width / 2);
    }

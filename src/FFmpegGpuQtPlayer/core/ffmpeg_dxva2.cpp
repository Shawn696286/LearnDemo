...
static int dxva2_retrieve_data(AVCodecContext* s, AVFrame* frame)
{
    ...
    calculate_display_rect(&rect, m_rtViewport.left, m_rtViewport.top, m_rtViewport.right - m_rtViewport.left,
                           m_rtViewport.bottom - m_rtViewport.top, frame->width, frame->height, frame->sample_aspect_ratio);
...
static int dxva2_alloc(AVCodecContext* s, HWND hwnd)
{
    ...
    if(!ctx->d3dlib)
    {
        LOGY_ERROR("Failed to load D3D9 library\n");
        goto fail;
    }
    ...
    if(!ctx->dxva2lib)
    {
        LOGY_ERROR("Failed to load DXVA2 library\n");
        goto fail;
    }
    ...
    if(!createD3D)
    {
        LOGY_ERROR("Failed to locate Direct3DCreate9\n");
        goto fail;
    }
    ...
    {
        LOGY_ERROR("Failed to locate DXVA2CreateDirect3DDeviceManager9\n");
        goto fail;
    }
    ...
    if(!ctx->d3d9)
    {
        LOGY_ERROR("Failed to create IDirect3D object\n");
        goto fail;
    }
    ...
    if(ist->hwaccel_device)
    {
        ...
        LOGY_ERROR("Using HWAccel device %d\n", adapter);
    }
...
static int dxva2_create_decoder(AVCodecContext* s)
{
    ...
    for(i = 0; dxva2_modes[i].guid; i++)
    {
        ...
        for(unsigned j = 0; j < target_count; j++)
        {
            ...
            if(format)
            {
                LOGY_ERROR("%s is supported for output\n", format->name);
            }
            else
            {
                LOGY_ERROR("%d is supported for output (%4.4s)\n", f, (const char*)&f);
            }
        }
    ...
    if(FAILED(hr))
    {
        LOGY_ERROR("Failed to create %d video surfaces\n", ctx->num_surfaces);
        goto fail;
    }
...
int dxva2_init2(AVCodecContext* s, HWND hwnd)
{
    ...
    {
        LOGY_ERROR("Unsupported H.264 profile for DXVA2 HWAccel: %d\n", s->profile);
        return AVERROR(EINVAL);
    }
    ...
    if(ret < 0)
    {
        LOGY_ERROR("Error creating the DXVA2 decoder\n");
        return ret;
    }
...
int dxva2_retrieve_data_call2(AVCodecContext* s, AVFrame* frame)
{
